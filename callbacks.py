...
class XiaotongCB(Callback):
    """
    This callback record loss and decay lr.
    """
    ...
    def __init__(self,
        ...
        """
        print('xiaotong call back init success........')
        self.filepath = filepath
        self.verbose = verbose
        self.factor = factor
...
class ReduceLRUponNan(Callback):
    ...
    def on_epoch_end(self, epoch: int, logs: Dict = None):
        ...
        """
        print('callback on epoch end success........')
        logs = logs or {}
        loss = logs.get('loss')
        last_saved_epoch, last_metric, last_file = self._get_checkpoints()
        if last_saved_epoch is not None:
